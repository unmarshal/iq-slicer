...
pub fn calculate_peak_power_db(samples: &[IqSample], planner: &mut FftPlanner<f32>) -> f32 {
    if samples.is_empty() {
...
/// Calculate peak power profile over time using FFT (finds strongest signal in any freq bin)
fn calculate_peak_power_profile(samples: &[IqSample], window_size: usize) -> Vec<f32> {
    if samples.len() < window_size {
        let mut planner = FftPlanner::new();
        return vec![calculate_peak_power_db(samples, &mut planner)];
    }

    let mut planner = FftPlanner::new();
    let mut profile = Vec::with_capacity(samples.len() / window_size);
    for chunk in samples.chunks(window_size) {
        profile.push(calculate_peak_power_db(chunk, &mut planner));
    }
    profile
}
...
/// Uses FFT peak power and hysteresis: triggers ON at threshold, OFF at threshold - 3dB
